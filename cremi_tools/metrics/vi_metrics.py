...
# VI-Scores from cremi_python:
# Evaluation code courtesy of Juan Nunez-Iglesias, taken from
# https://github.com/janelia-flyem/gala/blob/master/gala/evaluate.py


def voi(seg, gt, ignore_seg=[], ignore_gt=[0]):
    """Return the conditional entropies of the variation of information metric.
    ...
    """
    _, _, _, hxgy, hygx, _, _ = vi_tables(seg, gt, ignore_seg, ignore_gt)
    # false splits, false merges
    return hxgy.sum(), hygx.sum()
...
def contingency_table(seg, gt, ignore_seg=[0], ignore_gt=[0], norm=True):
    ...
    data = np.ones(len(gtr), dtype='uint64')
    for i in ignore_seg:
        ignored[segr == i] = True
    for j in ignore_gt:
        ignored[gtr == j] = True
    data[ignored] = 0
    cont = sparse.coo_matrix((data, (segr, gtr)), dtype='float64').tocsc()
    if norm:
        cont /= float(cont.sum())
    return cont
